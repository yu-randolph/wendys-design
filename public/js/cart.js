...
$(".cart").on("click", ".btn-update", function () {
  alert('TODO: Debug func');
  // for(var i=1; i<=cart.length; i++){
    // console.log('i:'+i);
    // var child = $('.cartContainer:nth-child('+i+')');
    // console.log(child);
    // if(cart.[i].quantity == ){
    //
    // }
...
function initCart(){
    ...
    var itemHTML = '<div class="item '+isAlter+'" id="cart-'+cart[i].id+'-'+cart[i].mealid+'" data-quantity="'+cart[i].quantity+'">'+
  						...
  						'<div class="col-md-6">'+
  							'<div class="input-group input-group-sm">'+
  								...
  								'<input type="text" class="form-control text-center digits cart-quantity" value="'+cart[i].quantity+'" name="quantity1">'+
  							'</div>'+
  						'</div>'+
  						'<div class="col-md-6 text-right">'+
  							'<div class="price">Php '+cart[i].price+'</div>'+
  						'</div>'+
  						'<div class="clearfix"></div>'+
  					'</div>';
  ...
  if(cart.length != 0 && subTotal < 250){
    ...
    $('.bottomTotal').append('<button type="button" class="btn btn-warning btn-sm" onclick="location.href=\'/wendys-design/public/checkout/2\'">'+
